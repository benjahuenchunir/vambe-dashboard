"""
Consolida los valores de canales_no_soportados_solicitados (text[] y JSONB)
a categorías normalizadas.

Uso:
    python -m scripts.consolidate_canales --table clients --batch-size 500
    python -m scripts.consolidate_canales --table clients --dry-run
"""

import argparse
from typing import Dict, List, Optional

from data import db
from config import load_settings

# ── Mapeo case-insensitive: valor detectado → categoría consolidada ──
MAPEO_CANALES: Dict[str, str] = {
    # Email
    "correo": "Email",
    "correo electrónico": "Email",
    "email": "Email",
    # SMS
    "sms": "SMS",
    # Llamadas / Telefonía
    "llamadas": "Llamadas",
    "telefonía básica": "Llamadas",
    "video llamada": "Llamadas",
    "videollamada": "Llamadas",
    # App Móvil
    "aplicación móvil": "App Móvil",
    # Chat Web
    "chat web": "Web",
    # Web
    "sitio web": "Web",
    "web": "Web",
    # Slack
    "slack": "Slack",
    # Intranet
    "intranet": "Intranet",
    # GPS / Tracking
    "gps en tiempo real": "GPS / Tracking",
    "seguimiento gps en tiempo real": "GPS / Tracking",
    "tracking en tiempo real de buses": "GPS / Tracking",
    # Apps de Delivery
    "apps de delivery": "Apps de Delivery",
    "plataformas de delivery": "Apps de Delivery",
    "rappi": "Apps de Delivery",
    "uber eats": "Apps de Delivery",
    # Pasarela de Pagos
    "paypal": "Pasarela de Pagos",
    # Otro
    "otro": "Otro",
}


def consolidar_elemento(valor: Optional[str]) -> Optional[str]:
    if not valor or not isinstance(valor, str):
        return valor
    key = valor.strip().lower().rstrip(".")
    return MAPEO_CANALES.get(key, "Otro")


def consolidar_array(arr: Optional[List[str]]) -> tuple[List[str], bool]:
    if not isinstance(arr, list):
        return arr if arr is not None else [], False

    nuevo = []
    cambio = False
    for item in arr:
        if not isinstance(item, str):
            nuevo.append(item)
            continue
        consolidado = consolidar_elemento(item)
        if consolidado != item:
            cambio = True
        if consolidado:  # nunca null, pero por si acaso
            nuevo.append(consolidado)

    # Eliminar duplicados preservando orden
    vistos = set()
    dedup = []
    for item in nuevo:
        if item not in vistos:
            vistos.add(item)
            dedup.append(item)

    return dedup, cambio or len(dedup) != len(nuevo)


def clean_jsonb_array(obj: dict, path: str) -> bool:
    keys = path.split(".")
    for key in keys[:-1]:
        if not isinstance(obj, dict) or key not in obj:
            return False
        obj = obj[key]
    last = keys[-1]
    if last not in obj or not isinstance(obj[last], list):
        return False

    nuevo, cambio = consolidar_array(obj[last])
    if cambio:
        obj[last] = nuevo
        return True
    return False


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Consolida canales_no_soportados_solicitados a categorías normalizadas"
    )
    parser.add_argument("--table", required=True, help="Nombre de la tabla")
    parser.add_argument(
        "--batch-size", type=int, default=500, help="Registros por página"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Solo cuenta, no escribe"
    )
    args = parser.parse_args()

    settings = load_settings()
    supabase = db.get_client(
        settings.supabase_url,
        settings.supabase_service_role_key,
    )

    print(f"▶ Escaneando tabla '{args.table}'...")
    print(f"  Modo: {'DRY-RUN' if args.dry_run else 'LIVE'}")
    print()

    actualizados, limpios, fallidos = 0, 0, 0
    distribucion: Dict[str, int] = {}
    offset = 0
    page = 1

    while True:
        query = (
            supabase.table(args.table)
            .select("id,canales_no_soportados_solicitados,raw_extraction")
            .order("id")
            .range(offset, offset + args.batch_size - 1)
        )

        response = query.execute()
        registros = getattr(response, "data", []) or []

        if not registros:
            break

        for reg in registros:
            reg_id = reg.get("id")
            if not reg_id:
                continue

            update_data = {}
            changed = False

            # ── 1. Columna plana: canales_no_soportados_solicitados ──
            original_arr = reg.get("canales_no_soportados_solicitados")
            if isinstance(original_arr, list):
                nuevo_arr, arr_cambio = consolidar_array(original_arr)
                if arr_cambio:
                    update_data["canales_no_soportados_solicitados"] = nuevo_arr
                    changed = True
                    for item in nuevo_arr:
                        distribucion[item] = distribucion.get(item, 0) + 1
                else:
                    for item in original_arr:
                        key = consolidar_elemento(item) or item
                        distribucion[key] = distribucion.get(key, 0) + 1

            # ── 2. JSONB raw_extraction ──
            raw = reg.get("raw_extraction")
            if isinstance(raw, dict):
                raw_changed = clean_jsonb_array(
                    raw, "necesidades_y_casos_uso.canales_no_soportados_solicitados"
                )
                if raw_changed:
                    update_data["raw_extraction"] = raw
                    changed = True

            if not changed:
                limpios += 1
                continue

            if args.dry_run:
                actualizados += 1
                continue

            try:
                result = (
                    supabase.table(args.table)
                    .update(update_data)
                    .eq("id", reg_id)
                    .execute()
                )
                if getattr(result, "data", None):
                    actualizados += 1
                else:
                    fallidos += 1
            except Exception as exc:
                print(f"  [ERROR] id={reg_id}: {exc}")
                fallidos += 1

        print(f"  Página {page}: {len(registros)} registros...")
        page += 1

        if len(registros) < args.batch_size:
            break
        offset += args.batch_size

    print()
    print("═" * 50)
    print(f"  ACTUALIZADOS : {actualizados}")
    print(f"  SIN CAMBIOS  : {limpios}")
    print(f"  FALLIDOS     : {fallidos}")
    print("═" * 50)
    print()
    print("  Distribución final de canales:")
    for canal, cnt in sorted(distribucion.items(), key=lambda x: -x[1]):
        print(f"    {canal:25s}: {cnt:5d}")


if __name__ == "__main__":
    main()